...
class Trader:
    ...
    def options_signal1(self, state, order_depths):
        ...
        default_options_offset = {
            "VOLCANIC_ROCK_VOUCHER_9500": 0.4914354792672384,
            "VOLCANIC_ROCK_VOUCHER_9750": 0.9098719026806066,
            "VOLCANIC_ROCK_VOUCHER_10000": 1.1625344879348631,
            "VOLCANIC_ROCK_VOUCHER_10250": 0.6929186395291254,
            "VOLCANIC_ROCK_VOUCHER_10500": 0.983989212420227
        }

        default_options_vol = {
            "VOLCANIC_ROCK_VOUCHER_9500": 0.41917531171421674, # 9.250213706500483,
            "VOLCANIC_ROCK_VOUCHER_9750": 0.9771898939085488, # 16.113672371581206,
            "VOLCANIC_ROCK_VOUCHER_10000": 1.871499550988819, # 22.150063341406153,
            "VOLCANIC_ROCK_VOUCHER_10250": 1.6293305311700836,
            "VOLCANIC_ROCK_VOUCHER_10500": 1.051607031941813, # 14.443010012913295
        }
        ...
        for x,y in self.options_spread_hist.items():
            ...
            rolling_mean = default_options_offset[x] # np.median(np.array(y))
            ...
            logger.print(f"ZSCORE: {z_score}, {y[-1]}, {rolling_std}, {prod}")

            # TRADE IF Z SCORE IS ABOVE 2
            if abs(z_score) > 2: # and rolling_std > default_options_vol[x]:

                # IMPLIED - MID PRICE is positive i.e. mid price is undervalued, so we buy
                if z_score > 2: res[prod] = 1      
                else: res[prod] = -1
            else:
                if rolling_std < default_options_vol[x] and abs(z_score) < 0.1: res[prod] = 2
                else: res[prod] = 0
    ...
    def options_signal2(self, state, order_depths):
        ...
        true_IV = [self.V_t(default_options_params[x][:3], m_t[x]) for x in range(5)]
        ...
        z_score = (np.array(true_IV) - np.array(cur_IV))/ np.array([default_options_params[x][3] for x in range(5)])
        ...
        for x in range(5):
            ...
            if rolling_STD < 1e-6:
                rolling_STD = default_options_params[x][3]
    ...
    def execute_options(self, state, order_depths):
        ...
        for x in products:
            ...
            if signal == 2 and abs(state.position.get(x, 0)) > 0 :
                price = int(self.mid_price(order_depths[x]))
                res[x] = Order(x, price, - state.position.get(x, 0))
    ...
    def run(self, state: TradingState):
        ...
        conversions = 0 # max(-10, min(10, conversions))
