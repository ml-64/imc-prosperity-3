...
class Trader:
    ...
    def create_spread_orders_basket1(
              ...
              synthetic_order_depth = self.synthetic_basket1_orders(order_depths)
              basket_vwap = self.vwap(basket_order_depth)
              synthetic_vwap = self.vwap(synthetic_order_depth)
    ...
    def calculate_basket2_spread(self, order_depths: Dict[str, OrderDepth]) -> float:
      basket1_vwap = self.vwap(order_depths[Product.PICNIC_BASKET1])
      basket2_vwap = self.vwap(order_depths[Product.PICNIC_BASKET2])
      djembe_vwap = self.vwap(order_depths[Product.DJEMBES])
      return basket1_vwap - djembe_vwap - 1.5 * basket2_vwap

    def create_spread_orders_basket2(self, order_depths, product, basket1_pos, spread_data):
      if Product.PICNIC_BASKET1 not in order_depths or Product.PICNIC_BASKET2 not in order_depths:
          return None

      spread = self.calculate_basket2_spread(order_depths)
      spread_data["spread_history"].append(spread)
      ...
      if zscore >= self.params[Product.SPREAD]["zscore_threshold"]:
          if basket1_pos != -self.params[Product.SPREAD]["target_position"]:
              return self.execute_dual_basket_trade(
                  -self.params[Product.SPREAD]["target_position"],
                  basket1_pos,
                  order_depths,
                  is_long=False  # short spread
              )
    ...
    def run(self, state: TradingState):
        ...
        if Product.SPREAD not in traderData:
            traderData[Product.SPREAD] = {
                "spread_history": [],
                "prev_zscore": 0,
            }
        ...
        if product.SPREAD_B2 not in traderData:
            traderData["SPREAD_B2"] = {
                "spread_history": [],
                "prev_zscore": 0,
