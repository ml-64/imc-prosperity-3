...
COPY_TARGETS = {
    "SQUID_INK": {
        "signals": [
            {"trader": "Charlie", "counterparty": "", "quantity": 0, "weight": 1.0, "watch_asset": "SQUID_INK"}
        ]
    },
}
...
class Trader:
    ...
    def __init__(self, params=None):
        if params is None:
            params = PARAMS
        self.params = params
        self.positions = {asset: 0 for asset in COPY_TARGETS}
        self.desired_position_pct = {asset: 0.0 for asset in COPY_TARGETS}
        self.price_history = {asset: [] for asset in COPY_TARGETS}
    ...
    def calculate_moving_average(self, asset: str, period: int = 50) -> Optional[float]:
        if len(self.price_history[asset]) < period:
            return None
        return float(np.mean(self.price_history[asset][-period:]))

    def should_copy_trade(self, trade, target_config, actual_trader) -> bool:
        return trade.buyer == actual_trader or trade.seller == actual_trader

    def should_copy_trade_with_sma(self, trade, target_config, actual_trader, moving_average):
        if not self.should_copy_trade(trade, target_config, actual_trader):
            return False
        price = trade.price
        if moving_average is not None:
            if abs(price - moving_average) / moving_average > 0.05:
                return False
        return True
    ...
    def basket_divergence(self, order_depths):
        ...
        z_score = (jams_price - croissant_price) / 54.85905218876869

        if abs(z_score) > 41.55:
    ...
    ##### INK+ ORDERS #####
    def process_asset(self, state: TradingState, asset: str, orders: Dict[str, List[Order]]):
        orders[asset] = []
        trade_made = "NONE"
        ...
        if best_bid > 0 and best_ask < float('inf'):
            ...
            self.price_history[asset].append(midpoint_price)
            if len(self.price_history[asset]) > 50:
                self.price_history[asset].pop(0)

        moving_average = self.calculate_moving_average(asset)
        ...
        for signal_config in COPY_TARGETS[asset]["signals"]:
            ...
            trades = state.market_trades.get(watch_asset, [])

            for trade in trades:
                if self.should_copy_trade_with_sma(trade, signal_config, actual_trader, moving_average):
                    adjustment = signal_config["weight"] * (-1 if inverse_copy else 1)
        ...
        target_position = int(self.desired_position_pct[asset] * self.POSITION_LIMITS[asset])
        position_difference = target_position - self.positions[asset]

        if position_difference > 0 and best_ask != float('inf'):
            buy_qty = min(position_difference, abs(order_depth.sell_orders[best_ask]))
            if buy_qty > 0:
                self.positions[asset] += buy_qty
                orders[asset].append(Order(asset, best_ask, buy_qty))
                trade_made = f"BUY {buy_qty} @ {best_ask}"

        elif position_difference < 0 and best_bid > 0:
            sell_qty = min(abs(position_difference), abs(order_depth.buy_orders[best_bid]))
            if sell_qty > 0:
                self.positions[asset] -= sell_qty
                orders[asset].append(Order(asset, best_bid, -sell_qty))
                trade_made = f"SELL {sell_qty} @ {best_bid}"
    ...
    def execute_ink_orders(self, order_depth, state):
        ...
        if abs(acceptable_price - i_price) > self.ink_vol * 2:
            ...
            if len(order_depth.sell_orders) != 0:
                ...
                for ask_price, ask_qty in sorted_asks:
                    if ask_price <= acceptable_price:
                        # ML: Maximum we can buy without breaching the long limit:
    ...
    def run(self, state: TradingState):
        ...
        # for asset in COPY_TARGETS:
        #     self.process_asset(state, asset, orders)
        # return orders, 0, ""
        ...
        return result, conversions, traderData, orders
